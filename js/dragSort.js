...
function DragSort(box, callBack) {
	...
	function insertAfter(newChild,refChild) {
		var parElem=refChild.parentElement;
	 ...
	 * 
	 */
	function canncelAni(type) {
		_this.elementsDom = _this.box.querySelectorAll('.dragSort-js')
		for (var s = 0; s < _this.elementsDom.length; s++) {
			if (type == true){
				_this.elementsDom[s].classList.remove('znavIcons-ani')
			} else {
				_this.elementsDom[s].classList.add('znavIcons-ani')
			}
		}
	}
		...
		for(var d = 0; d < this.elementsDom.length; d++) {
			var dom = new DragBox(this.elementsDom[d], {
				...
				dragEnd: function(o) {
					...
					if (_this.currentObj) {
						if (_this.currentObj.ori == 'right') {
							...
							console.log(o.self, _this.currentObj.obj.dom)
						} else if (_this.currentObj.ori == 'left') {
							_this.box.insertBefore(o.self, _this.currentObj.obj.dom)
						}
					} else {
						console.log(_this.box, o.self, _this.virDom)
				...
				holdpress: function (o) {
					...
					console.log(o.self, _this.virDom, o.self.parentElement, o.self.nextSibling)
					...
					o.self.style.left = o.startX - (o.startX - o.that.oLeft) + 5 + 'px'
					o.self.style.top = o.startY - (o.startY - o.that.oTop) + 5 + 'px'
				}
			})
